...
CONTAINS
  ...
  FUNCTION stab_fn_mom(StabilityMethod,ZL,zl_f) RESULT(psym)
    ...
    ELSEIF(zL<-neut_limit) THEN    !Unstable
       ...
       ELSEIF(StabilityMethod==3)THEN !     campbell & norman eqn 7.26 An Introduction to Environmental Biophysics(1997)
          psym=0.6*(2)*LOG((1+(1-16*zl_f)**0.5)/2)
          !here coefficient is (0.6) but in book is (-0.6)
       ...
       ELSEIF(StabilityMethod==6)THEN !     Foken and Skeib (1983) Hogstrom (1988)
          IF(zl_f>=-0.06)THEN
             x=1
          ELSE
             x=((-1)*zl_f/0.06)**(-0.25)
    ...
    ELSEIF(zL>neut_limit) THEN            !Stable
       ...
       ELSEIF(StabilityMethod==3)THEN ! campbell & norman eqn 7.27 p 97 An Introduction to Environmental Biophysics(1997)
          psym=(-6)*LOG(1+zl_f)
          !here coefficient is (-6) but in book is 6
       ENDIF
    ENDIF
    RETURN
  ...
  FUNCTION stab_fn_heat(StabilityMethod,ZL,zl_f) RESULT (psyh)
    ...
    ELSEIF(zL<-neut_limit) THEN     ! Unstable
       IF(StabilityMethod==3)THEN
          !campbell & norman eqn 7.26 An Introduction to Environmental Biophysics(1997)
          psyh=(2)*LOG((1+(1-16*zl_f)**0.5)/2)
       ELSE

          IF(StabilityMethod==4)THEN ! Businger et al (1971) modifed  Hogstrom (1988)
             x=0.95*(1.-11.6*zl_f)**(-0.5)
          ELSEIF(StabilityMethod==7) THEN ! Dyer & Bradley (1982)
             x=(1.-(14.*ZL))**(-0.5)
          ELSEIF(StabilityMethod==2)THEN ! Dyer 1974 X=(1.-(16.*ZL))**(0.5)modified Hosgstrom
             x=0.95*(1.-15.2*zl_f)**(-0.5)
          ENDIF
          PSYH=2*LOG((1+x**2)/2)   !Brutasert (1982) eqn 4.51 value too large when x->-5
