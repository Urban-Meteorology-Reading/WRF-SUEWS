...
real      ,DIMENSION(grid%sm31:grid%em31,grid%sm32:grid%em32,grid%spec_bdy_width,num_tracer)           :: tracer_btye
   ...
   IF( couple ) THEN
     ...
     DO i = max(ids,ips),min(ide-1,ipe)
       mutf_2(i,k,j) = grid%Mub(i,j) + grid%Mu_2(i,j)
       muwt_2(i,k,j) = (grid%Mub(i,j) + grid%Mu_2(i,j))/grid%msfty(i,j) 
     ENDDO
     ENDDO
     ENDDO
     ...
     DO k = kps,kpe-1
     DO i = max(ids,ips),min(ide-1,ipe)
       muth_2(i,k,j) = grid%mub(i,j) + grid%mu_2(i,j)
     ENDDO
     ENDDO
     ENDDO
     ...
     DO k = kps,kpe-1
     DO i = max(ids,ips),min(ide-1,ipe)
       muut_2(i,k,j) = 0.5*(grid%mub(i,j)+grid%mub(i-1,j) + grid%mu_2(i,j) + grid%mu_2(i-1,j))/grid%msfuy(i,j) 
       muvt_2(i,k,j) = 0.5*(grid%mub(i,j)+grid%mub(i,j-1) + grid%mu_2(i,j) + grid%mu_2(i,j-1))/grid%msfvx(i,j) 
     ENDDO
     ENDDO
     ENDDO
     ...
     IF ( config_flags%nested .or. config_flags%specified .or. config_flags%polar ) THEN
       ...
       IF ( jpe .eq. jde ) THEN
         ...
         DO k = kps,kpe-1
         DO i = max(ids,ips),min(ide-1,ipe)
           muvt_2(i,k,j) = (grid%mub(i,j-1) + grid%mu_2(i,j-1))/grid%msfvx(i,j) 
         ENDDO
         ENDDO
       ENDIF
       IF ( ipe .eq. ide .AND. .NOT. config_flags%periodic_x ) THEN
         ...
         DO j = max(jds,jps),min(jde-1,jpe)
         DO k = kps,kpe-1
           muut_2(i,k,j) = (grid%mub(i-1,j) + grid%mu_2(i-1,j))/grid%msfuy(i,j) 
         ENDDO
         ENDDO
       ENDIF
   ...
   ELSE
     ...
     DO k = kps,kpe
     DO i = max(ids,ips),min(ide-1,ipe)
       mutf_2(i,k,j) = 1./(grid%Mub(i,j) + grid%Mu_2(i,j))
       muwt_2(i,k,j) = grid%msfty(i,j)/(grid%Mub(i,j) + grid%Mu_2(i,j)) 
     ENDDO
     ENDDO
     ENDDO
     ...
     DO k = kps,kpe-1
     DO i = max(ids,ips),min(ide-1,ipe)
       muth_2(i,k,j) = 1./(grid%mub(i,j) + grid%mu_2(i,j))
     ENDDO
     ENDDO
     ENDDO
     ...
     DO k = kps,kpe-1
     DO i = max(ids,ips),min(ide-1,ipe)
       muut_2(i,k,j) = 2.*grid%msfuy(i,j)/(grid%mub(i,j)+grid%mub(i-1,j) + grid%mu_2(i,j) + grid%mu_2(i-1,j)) 
     ENDDO
     ENDDO
     ENDDO
     ...
     DO k = kps,kpe-1
     DO i = max(ids,ips),min(ide-1,ipe)
       muvt_2(i,k,j) = 2.*grid%msfvx(i,j)/(grid%mub(i,j)+grid%mub(i,j-1) + grid%mu_2(i,j) + grid%mu_2(i,j-1)) 
     ENDDO
     ENDDO
     ENDDO
     ...
     IF ( config_flags%nested .or. config_flags%specified .or. config_flags%polar ) THEN
       ...
       IF ( jpe .eq. jde ) THEN
         ...
         DO k = kps,kpe-1
         DO i = max(ids,ips),min(ide-1,ipe)
           muvt_2(i,k,j) = grid%msfvx(i,j)/(grid%mub(i,j-1) + grid%mu_2(i,j-1)) 
         ENDDO
         ENDDO
       ENDIF
       IF ( ipe .eq. ide .AND. .NOT. config_flags%periodic_x ) THEN
         ...
         DO j = max(jds,jps),min(jde-1,jpe)
         DO k = kps,kpe-1
           muut_2(i,k,j) = grid%msfuy(i,j)/(grid%mub(i-1,j) + grid%mu_2(i-1,j)) 
         ENDDO
         ENDDO
       ENDIF
