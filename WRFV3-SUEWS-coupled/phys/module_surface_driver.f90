...
CONTAINS
   ...
   SUBROUTINE surface_driver(                                         &
     ...
     &          ,chklowq,dt,dt_prev,dx,dz8w,dzs,glw                           &
   ...
   REAL, OPTIONAL, DIMENSION( 3 ) , INTENT(IN) :: OHMcoef
...
'SUEWS scheme cannot be used with fractional seaice')
        ...
        CALL wrf_debug(100,'in SUEWS')
          ...
          CALL suewsdrv(year, day_int, hour, minute, second, xtime, OHMcoef, &
...
'Lacking arguments for P-X LSM in surface driver')
     ...
     IF (MYJ) call wrf_error_fatal3("<stdin>",3343,&
