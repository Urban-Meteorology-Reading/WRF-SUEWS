...
def parameters(path_dir_output, path_csv_phenol, path_runcontrol, first_day_str):
    ...
    for path_wrfinput in list_path_wrfinput:
        ...
        for i in range(a["urban"].shape[0]):
            for j in range(a["urban"].shape[1]):
                ...
                ###########################################################################
                # TODO #84: too London specific.
                if a["urban"][i, j] > 0.60:
                    ds_base = change_soil_moisture(
                        ds_base, path_dir_output, "London", i, j
                    )
                ###########################################################################
                # TODO #84: too Swindon specific.
                elif (a["urban"][i, j] > 0.16) and (a["urban"][i, j] <= 0.60):
                    ds_base = change_soil_moisture(
                        ds_base, path_dir_output, "Swindon", i, j
                    )
                ###########################################################################
                ...
                ###########################################################################
                    ...
                    if (fr_paved >= 0.18105 and fr_paved < 0.27158) and (
                        fr_build >= 0.204301 and fr_build < 0.27240233
                    ):
                        ds_base = change_soil_moisture(
                            ds_base, path_dir_output, "G1", i, j
                        )
                    elif (fr_paved >= 0.18105 and fr_paved < 0.27158) and (
                        fr_build >= 0.13620117 and fr_build < 0.204301
                    ):
                        ds_base = change_soil_moisture(
                            ds_base, path_dir_output, "G2", i, j
                        )
                    elif (fr_paved >= 0.18105 and fr_paved < 0.27158) and (
                        fr_build >= 0.06810058 and fr_build < 0.13620117
                    ):
                        ds_base = change_soil_moisture(
                            ds_base, path_dir_output, "G3", i, j
                        )
                    elif (fr_paved >= 0.27158 and fr_paved < 0.36211554) and (
                        fr_build >= 0.06810058 and fr_build < 0.13620117
                    ):
                        ds_base = change_soil_moisture(
                            ds_base, path_dir_output, "G4", i, j
                        )
                    else:
                        ds_base = change_soil_moisture(
                            ds_base, path_dir_output, "Grass", i, j
