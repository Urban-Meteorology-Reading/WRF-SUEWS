...
def modify_London(path_dir_output, path_dir_data, file_to_change):
    ...
    cd = ~np.isnan(new_0)
    ds_base["LANDUSEF"].values[0, 12, :, :][cd] = (
        new_all["Fr_Paved"][cd] + new_all["Fr_Bldgs"][cd]
    )
    ...
    ds_base["PAVED_RATIO"].values[0, :, :][cd][
        ds_base["PAVED_RATIO"].values[0, :, :][cd] > 1
    ] = 0
    ...
    # Modifying the QF coefficients to be more representative
    w1s = [0.1446, 0, 0.0037]
    w2s = [0.133, 0, 0.0038]
